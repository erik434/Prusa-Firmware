...
bool check_commands()
{
  ...
  while (buflen)
  {
    if ((code_seen_P(PSTR("M84"))) || (code_seen_P(PSTR("M 84"))))
...
void process_commands()
{
  ...
  */
  ...
  { // M0 and M1 - (Un)conditional stop - Wait for user button press on LCD
...
void manage_inactivity(bool ignore_stepper_queue /*=false*/) // default argument set in Marlin.h
{
  ...
  if (mmu_enabled == false)
  {
    ...
    if (!PRINTER_ACTIVE)
      bInhibitFlag = (menu_menu == lcd_menu_show_sensors_state); // Block Filament sensor actions if PRINTER is not active and Support::SensorInfo menu active
#ifdef IR_SENSOR_ANALOG
...
{
  if (!eeprom_read_byte((unsigned char *)EEPROM_TEMP_CAL_ACTIVE))
    return 0;
...
void stop_and_save_print_to_ram(float z_move, float e_move)
{
  ...
  if (pos_invalid)
    saved_pos[X_AXIS] = X_COORD_INVALID;
